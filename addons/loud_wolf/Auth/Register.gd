...
func _ready():
	LoudWolf.check_auth_ready()
	LoudWolf.Auth.sw_registration_complete.connect(_on_registration_complete)
	LoudWolf.Auth.sw_registration_user_pwd_complete.connect(_on_registration_user_pwd_complete)
...
func _on_RegisterButton_pressed() -> void:
	...
	LoudWolf.Auth.register_player(player_name, email, password, confirm_password)
...
func _on_RegisterUPButton_pressed() -> void:
	...
	var password = $"FormContainer/MainFormContainer/FormInputFields/Password".text
	var confirm_password = $"FormContainer/MainFormContainer/FormInputFields/ConfirmPassword".text
	LoudWolf.Auth.register_player_user_password(player_name, password, confirm_password)
	show_processing_label()
...
func registration_success() -> void:
	...
	if ("email_confirmation_scene" in LoudWolf.auth_config) and (LoudWolf.auth_config.email_confirmation_scene) != "":
		SWLogger.info("registration succeeded, waiting for email verification...")
		scene_name = LoudWolf.auth_config.email_confirmation_scene
	else:
		SWLogger.info("registration succeeded, logged in player: " + str(LoudWolf.Auth.logged_in_player))
