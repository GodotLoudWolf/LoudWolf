...
func _ready():
	...
	LoudWolf.Auth.sw_request_password_reset_complete.connect(_on_send_code_complete)
	LoudWolf.Auth.sw_reset_password_complete.connect(_on_reset_complete)
	if "login_scene" in LoudWolf.Auth:
		login_scene = LoudWolf.Auth.login_scene
...
func _on_PlayerNameSubmitButton_pressed() -> void:
	...
	LoudWolf.Auth.request_player_password_reset(player_name)
...
func _on_NewPasswordSubmitButton_pressed() -> void:
	...
	LoudWolf.Auth.reset_player_password(player_name, code, password, confirm_password)
