...
func _ready():
	...
	print("LoudWolf.Scores.ldboard_config: " + str(LoudWolf.Scores.ldboard_config))
	var scores = LoudWolf.Scores.scores
	#var scores = []
	if ld_name in LoudWolf.Scores.leaderboards:
		scores = LoudWolf.Scores.leaderboards[ld_name]
	var local_scores = LoudWolf.Scores.local_scores
	...
	if len(scores) > 0: 
		...
		var sw_result = await LoudWolf.Scores.get_scores().sw_get_scores_complete
...
func render_board(scores: Array, local_scores: Array) -> void:
	...
	if ld_name in LoudWolf.Scores.ldboard_config and is_default_leaderboard(LoudWolf.Scores.ldboard_config[ld_name]):
