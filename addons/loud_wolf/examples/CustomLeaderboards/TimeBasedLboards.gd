...
func _ready():
	LoudWolf.Scores.sw_get_scores_complete.connect(_on_scores_received)
	
	#var scores = LoudWolf.Scores.scores
	add_loading_scores_message()
	var sw_result = LoudWolf.Scores.get_scores(10, "main")
...
func _on_CloseButton_pressed() -> void:
	var scene_name = LoudWolf.scores_config.open_scene_on_close
	SWLogger.info("Closing LoudWolf leaderboard, switching to scene: " + str(scene_name))
...
func _on_scores_received(get_scores_result: Dictionary) -> void:
	...
	if ld_name == "main":
		LoudWolf.Scores.get_scores(10, "Weekly")
		#LoudWolf.Scores.get_scores(10, "Weekly", -1)
	elif ld_name == "Weekly":
		LoudWolf.Scores.get_scores(10, "Monthly")
	else:
		#print("LoudWolf.Scores.leaderboards: " + str(LoudWolf.Scores.leaderboards))
		var ld_scores = []
		for i in [0, 1, 2]:
			if ld_names[i] in LoudWolf.Scores.leaderboards:
				ld_scores.append(LoudWolf.Scores.leaderboards[ld_names[i]])
			#elif (ld_names[i] + ";-1") in LoudWolf.Scores.leaderboards_past_periods:
			#	ld_scores.append(LoudWolf.Scores.leaderboards_past_periods[(ld_names[i] + ";-1")])
			else:
				ld_scores.append([])
		hide_message()
