...
func save_player_data(player_name: String, player_data: Dictionary, overwrite: bool=true) -> Node:
	if player_name == "":
		SWLogger.error("player_name cannot be empty when calling LoudWolf.Players.save_player_data(player_name, player_data)")
...
func _on_SavePlayerData_request_completed(result, response_code, headers, body) -> void:
	...
	if is_instance_valid(SavePlayerData): 
		LoudWolf.free_request(wrSavePlayerData, SavePlayerData)
	...
	if status_check:
		...
		if json_body.success:
			...
			SWLogger.error("LoudWolf save player data failure: " + str(json_body.error))
		sw_save_player_data_complete.emit(sw_result) 


func get_player_data(player_name: String) -> Node:
	if player_name == "":
		SWLogger.error("player_name cannot be empty when calling LoudWolf.Players.get_player_data(player_name)")
	else:
		var prepared_http_req = LoudWolf.prepare_http_request()
...
func _on_GetPlayerData_request_completed(result, response_code, headers, body) -> void:
	...
	if is_instance_valid(GetPlayerData): 
		LoudWolf.free_request(wrGetPlayerData, GetPlayerData)
